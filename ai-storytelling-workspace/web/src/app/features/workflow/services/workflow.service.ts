...
export class WorkflowService {
  ...
  startWorkflow(projectId: string): Observable<WorkflowActionResponse> {
    return this.api.post<WorkflowActionResponse>(`/workflow/start/${projectId}`, {});
  }

  pauseWorkflow(projectId: string): Observable<WorkflowActionResponse> {
    return this.api.post<WorkflowActionResponse>(`/workflow/pause/${projectId}`, {});
  }

  resumeWorkflow(projectId: string): Observable<WorkflowActionResponse> {
    return this.api.post<WorkflowActionResponse>(`/workflow/resume/${projectId}`, {});
  }

  getStatus(projectId: string): Observable<Partial<WorkflowState>> {
    return this.api.get<Partial<WorkflowState>>(`/workflow/status/${projectId}`);
  }
